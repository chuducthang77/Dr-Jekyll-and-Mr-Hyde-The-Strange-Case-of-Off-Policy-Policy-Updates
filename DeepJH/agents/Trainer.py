...
from agents.logger import SummaryWriter_X
...
class Trainer(object):
    ...
    def __init__(self, config, agents):
        ...
        if hasattr(config, 'log_path'):
            self.tensorboard = SummaryWriter_X(config.log_path, flush=True)
        else:
    ...
    def create_agent_to_agent_group_dictionary(self):
        ...
        agent_to_agent_group_dictionary = {
            ...
            "J": "Actor_Critic_Agents",
            "JH": "Actor_Critic_Agents",
            "HRL": "HRL",
            "Model_HRL": "HRL",
            "DIAYN": "DIAYN",
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        agent_round = 1
        logging_interval = getattr(self.config, 'logging_interval', 1)
        for run in range(self.config.runs_per_agent):
            ...
            print(f"AGENT NAME: {agent_name} (number {agent_number}). Round {agent_round} out of {self.config.runs_per_agent}")
            # print("\033[1m" + "{}.{}: {}".format(agent_number, agent_round, agent_name) + "\033[0m", flush=True)
            agent = agent_class(agent_config)
            agent.set_agent_round_and_tb(agent_round, self.tensorboard)
            ...
            if agent.state_actions_found:
                print('State actions discovered:', agent.state_actions_found)
                agent_results[-1].append(agent.state_actions_found)
            ...
            if self.config.visualise_individual_results:
                ...
                plt.show()
            agent_round += 1
            if False and self.tensorboard:
                for i, score in enumerate(game_scores):
                    if i % logging_interval == 0:
                        self.tensorboard.add_scalars('rolling_score', {f'{agent_name}_{run}': rolling_scores[i]}, i)
                        self.tensorboard.add_scalars('score', {f'{agent_name}_{run}': score}, i)
        self.results[agent_name] = agent_results

        def log_tb_score(res, title=''):
            _, mean_results, _ = self.get_mean_and_standard_deviation_difference_results(res)
            for i, score in enumerate(mean_results):
                if i % logging_interval == 0:
                    self.tensorboard.add_scalars(title, {f'{agent_name}': score}, i)
        if False and self.tensorboard:
            log_tb_score([results[1] for results in agent_results], title='mean_rolling_score')
            log_tb_score([results[0] for results in agent_results], title='mean_score')
